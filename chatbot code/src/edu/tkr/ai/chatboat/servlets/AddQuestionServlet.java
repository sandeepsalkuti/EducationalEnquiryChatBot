...
public class AddQuestionServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(isUploaded)
		{
			...
			if(result==1)
			{
				int role=AppDAO.getRole(username);
				
				if(role==2)
				{
					response.sendRedirect("addquestions1.jsp?status=succes");
				}
				else
				{
					response.sendRedirect("addquestions.jsp?status=succes");
				}
			...
			else
			{
				...
				if(role==2)
				{
					response.sendRedirect("addquestions1.jsp?status=failed");
				}
				else
				{
					response.sendRedirect("addquestions.jsp?status=failed");
				}
			}
		}
		else
